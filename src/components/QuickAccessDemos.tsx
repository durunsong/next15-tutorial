...
import { ExternalLink, Server, Globe, Zap, Monitor, Activity } from 'lucide-react';
    ...
    <div className="my-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {demoPages.map(demo => {
          ...
          return (
            <div key={demo.id} className={`border rounded-lg p-4 ${colorClasses.bg} ${colorClasses.border}`}>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-yellow-800 dark:text-yellow-200 text-sm">
              访问演示页面时，建议按 <kbd className="px-1 py-0.5 bg-yellow-200 dark:bg-yellow-800 rounded text-xs">F12</kbd> 打开开发者工具，
              切换到 <strong>Network</strong> 面板，然后刷新页面观察不同渲染模式的网络请求瀑布图和加载时序。
              您会发现 SSG 加载最快，CSR 有更多 API 请求，SSR 在服务器端完成渲染等不同特征。
            </p>
          </div>
        </div>
      </div>
